...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 跳转登录会再执行 PasswordToken 登录；若请求仍带旧 JWT，先 bind 旧用户再换人会放大「换账号权限错乱」的体感，故该接口不预绑定 Token 身份
        if (token != null && !isJumpLoginRequest(request)) {
            try {
    ...
    /** 与 {@code PermissionAuthController#jumpLogin} 对应；避免旧 Token 在本请求内先绑定身份。 */
    private static boolean isJumpLoginRequest(HttpServletRequest request) {
        String uri = request.getRequestURI();
        return uri != null && uri.contains("/permission-auth/jump-login");
    }
}